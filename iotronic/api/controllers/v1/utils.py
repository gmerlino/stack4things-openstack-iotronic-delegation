...
def get_rpc_user(user_ident):
    """Get the RPC user from the user uuid or logical name.

    :param user_ident: the UUID or logical name of a user.

    :returns: The RPC User.
    :raises: InvalidUuidOrName if the name or uuid provided is not valid.
    :raises: UserNotFound if the user is not found.
    """

    if uuidutils.is_uuid_like(user_ident):
        return objects.User.get_by_uuid(pecan.request.context, user_ident)

    else:
        return objects.User.get_by_name(pecan.request.context, user_ident)

    raise exception.InvalidUuidOrName(name=user_ident)

    raise exception.UserNotFound(user=user_ident)


def get_rpc_role(role_ident):
    """Get the RPC role from logical name.

    :param role_ident: logical name of a role.

    :returns: The RPC Role.
    :raises: RoleNotFound if the role is not found.
    """
    return objects.Role.get_by_name(pecan.request.context, role_ident)

    raise exception.RoleNotFound(role=role_ident)


def get_rpc_current_delegation(user_uuid, node_ident):
    """Get the RPC delegation from the delegation uuid or logical name.

    :param delegation: the UUID or logical name of a delegation.

    :returns: The RPC delegation.
    """
    return objects.Delegation.get_by_user_node(pecan.request.context,
                                               user_uuid, node_ident)


def get_rpc_delegation(delegation_uuid):
    """Get the RPC delegation from the delegation uuid or logical name.
    ...
    :returns: The RPC delegation.
    """
    return objects.Delegation.get_by_uuid(pecan.request.context,
                                          delegation_uuid)


def check_role_assignment(delegator_role, delegated_role, type):
    """Ceck delegation role assignment.
    """
    objects.Delegation.check_role_assignment(pecan.request.context,
                                             delegator_role,
                                             delegated_role, type)
