...
class PublicPluginsController(rest.RestController):
    ...
    def _get_plugins_collection(self, authorized_plugins, marker, limit,
                                sort_key, sort_dir,
                                fields=None):
        ...
        plugins = objects.Plugin.list(pecan.request.context,
                                      authorized_plugins, limit, marker_obj,
    ...
    def get_all(self, marker=None,
        ...
        """
        authorized_plugins = authorization.authorize('plugin:get')

        if fields is None:
            fields = _DEFAULT_RETURN_FIELDS
        return self._get_plugins_collection(authorized_plugins, marker,
...
class PluginsController(rest.RestController):
    ...
    def get_one(self, plugin_ident, fields=None):
        ...
        if not rpc_plugin.public:
            authorization.authorize('plugin:get_one', rpc_plugin.uuid)
    ...
    def post(self, Plugin):
        ...
        """
        authorization.authorize('plugin:create')
        ...
        new_Plugin.owner = pecan.request.context.user_id
        ...
        new_Delegation.node = new_Plugin.uuid
        new_Delegation.type = 'plugin'
    ...
    def delete(self, plugin_ident):
        ...
        rpc_plugin = api_utils.get_rpc_plugin(plugin_ident)
        authorization.authorize('plugin:delete', rpc_plugin.uuid)
        pecan.request.rpcapi.destroy_plugin(pecan.request.context,
                                            rpc_plugin.uuid)
        # remove all plugin delegations
        objects.Delegation.destroy_by_node_uuid(
            pecan.request.context, rpc_plugin.uuid, 'plugin')
    ...
    def patch(self, plugin_ident, val_Plugin):
        ...
        rpc_plugin = api_utils.get_rpc_plugin(plugin_ident)
        authorization.authorize('plugin:update', rpc_plugin.uuid)
