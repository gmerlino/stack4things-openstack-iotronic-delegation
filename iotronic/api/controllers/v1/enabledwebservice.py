...
class EnabledWebservicesController(rest.RestController):
    ...
    def _get_EnabledWebservices_collection(self, auth_board_webservices,
        ...
        EnabledWebservices = objects.EnabledWebservice.list(
            pecan.request.context, auth_board_webservices, limit,
            marker_obj,
            sort_key=sort_key,
            sort_dir=sort_dir,
            filters=filters)
    ...
    def get_all(self, marker=None,
        ...
        """
        auth_board_webservices = authorization.authorize(
            'board:webservice_get')
        project_id = pecan.request.context.project_id

        if fields is None:
            fields = _DEFAULT_RETURN_FIELDS
        return self._get_EnabledWebservices_collection(auth_board_webservices,
                                                       marker, limit,
                                                       sort_key, sort_dir,
