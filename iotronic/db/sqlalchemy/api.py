...
class Connection(api.Connection):
    ...
    def _add_users_filters(self, query, filters):
        if filters is None:
            filters = []
        if 'base_role' in filters:
            query = query.filter(models.User.base_role == filters['base_role'])
        return query

    def _add_roles_filters(self, query, filters):
        if filters is None:
            filters = []
        if 'permission' in filters:
            query = query.filter(
                filters['permission'] in models.Role.permissions)
        return query

    def _add_boards_filters(self, query, filters, authorized_boards):
        if filters is None:
            filters = []
        ...
        # if 'uuid' in filters:
        #    query = query.filter(models.Board.uuid == filters['uuid'])
        query = query.filter(models.Board.uuid.in_(authorized_boards))

        return query

    def _add_plugins_filters(self, query, filters, authorized_plugins):
        if filters is None:
            filters = []

        if 'with_public' in filters and filters['with_public']:
            query = query.filter(
                or_(models.Plugin.public == 1,
                    models.Plugin.uuid.in_(authorized_plugins)))
        elif 'public' in filters and filters['public']:
            query = query.filter(
                or_(models.Plugin.public == 1,
                    models.Plugin.uuid.in_(authorized_plugins)))
        else:
            query = query.filter(models.Plugin.uuid.in_(authorized_plugins))
        return query

    def _add_services_filters(self, query, filters, authorized_services):
        if filters is None:
            filters = []

        # if 'owner' in filters:
        #    query = query.filter(models.Plugin.owner == filters['owner'])
        query = query.filter(models.Service.uuid.in_(authorized_services))
        return query

    def _add_enabled_webservices_filters(self, query, filters,
                                         authorized_board_webservices):
        if filters is None:
            filters = []
        ...
        query = query.filter(models.EnabledWebservice.board_uuid.in_(
            authorized_board_webservices))
        return query

    def _add_webservices_filters(self, query, filters, authorized_webservices):
        # if filters is None:
        #    filters = []
        if 'project_id' in filters:
            query = query.join(models.Board,
        ...
        query = query.filter(
            models.Webservice.uuid.in_(authorized_webservices))
        return query

    def _add_fleets_filters(self, query, filters, authorized_fleets):
        if filters is None:
            filters = []

        if 'project' in filters:
            query = query.filter(models.Fleet.project == filters['project'])
        query = query.filter(models.Fleet.uuid.in_(authorized_fleets))
        return query
    ...
    def _add_ports_filters(self, query, filters, authorized_ports):
        if filters is None:
            filters = []
        ...
        query = query.filter(models.Port.uuid.in_(authorized_ports))

    def _do_update_delegation(self, delegation_id, values):
        session = get_session()
        with session.begin():
            query = model_query(models.Delegation, session=session)
            query = add_identity_filter(query, delegation_id)
            try:
                ref = query.with_lockmode('update').one()
            except NoResultFound:
                raise exception.DelegationNotFound()
            ref.update(values)
        return ref
    ...
    def _do_update_role(self, role_id, values):
        session = get_session()
        with session.begin():
            query = model_query(models.Role, session=session)
            query = query.filter_by(name=role_id)
            try:
                ref = query.with_lockmode('update').one()
            except NoResultFound:
                raise exception.RoleNotFound(role=role_id)
    ...
    def get_node_delegations(self, user_uuid=None, type=None, node=None):
        if node:
            query = model_query(models.Delegation).filter_by(
                type=type, node=node)
        else:
            query = model_query(models.Delegation).filter_by(
                delegated=user_uuid, type=type)
        try:
            query = db_utils.paginate_query(
                query, models.Delegation, None, ['id'])
            return query.all()
        except Exception as ex:
            raise exception.TemporaryFailure(str(ex))

    def get_node_delegation(self, user_uuid, type, target):
        query = model_query(models.Delegation).filter_by(
            delegated=user_uuid, node=target)
        try:
            query = db_utils.paginate_query(
                query, models.Delegation, None, ['id'])
            return query.one()
    ...
    def get_role_permissions(self, role):
        query = model_query(models.Role).filter_by(name=role)
        try:
            query = db_utils.paginate_query(query, models.Role, None, ['id'])
            return query.one()
        except NoResultFound:
            raise exception.DelegationNotFound()

    def get_user_details(self, user_uuid):
        query = model_query(models.User).filter_by(uuid=user_uuid)
        try:
            query = db_utils.paginate_query(query, models.User, None, ['id'])
            return query.one()
        except NoResultFound:
    ...
    def create_delegation(self, values):
        ...
        try:
            ...
            raise exception.DelegationAlreadyExists()
        return delegation

    def update_delegation(self, delegation_id, values):
        for val in values:
            if val != 'role':
                msg = _("You can update only role for an existing Delegation.")
                raise exception.InvalidParameterValue(err=msg)
        try:
            return self._do_update_delegation(delegation_id, values)
    ...
    def destroy_delegation_by_node(self, node_id, type):
        session = get_session()
        with session.begin():
            query = model_query(models.Delegation).filter_by(
    ...
    def get_delegation_list(self, node_delegations, filters=None, limit=None,
                            marker=None, sort_key=None,
                            sort_dir=None):
        query = model_query(models.Delegation)
        query = query.filter(models.Delegation.node == node_delegations)
        return _paginate_query(models.Delegation, limit, marker,
                               sort_key, sort_dir, query)

    def get_delegation_by_uuid(self, delegation_uuid):
        query = model_query(models.Delegation).filter_by(uuid=delegation_uuid)
        try:
            return query.one()
        except NoResultFound:
            raise exception.DelegationNotFound()
    ...
    def get_delegation_by_id(self, delegation_id):
        query = model_query(models.Delegation).filter_by(id=delegation_id)
        try:
            return query.one()
        except NoResultFound:
            raise exception.DelegationNotFound()

    def get_delegation_by_user_node(self, user_uuid, node_ident):
        query = model_query(models.Delegation).filter_by(
            delegated=user_uuid, node=node_ident)
        try:
            return query.one()
        except NoResultFound:
            raise exception.DelegationNotFound()
    ...
    def get_user_list(self, filters=None, limit=None,
                      sort_key=None, sort_dir=None):
        query = model_query(models.User)
        query = self._add_users_filters(query, filters)
        return _paginate_query(models.User, limit, None,
                               sort_key, sort_dir, query)
    ...
    def get_user_by_name(self, user_name):
        query = model_query(models.User).filter_by(name=user_name)
        try:
            return query.one()
        except NoResultFound:
    ...
    def get_role_list(self, filters=None, limit=None,
                      sort_key=None, sort_dir=None):
        query = model_query(models.Role)
        query = self._add_roles_filters(query, filters)
        return _paginate_query(models.Role, limit, None,
                               sort_key, sort_dir, query)

    def get_ops_list(self, filters=None, limit=None,
                     sort_key=None, sort_dir=None):
        query = model_query(models.Operation)
        return _paginate_query(models.Operation, limit, None,
                               sort_key, sort_dir, query)

    def create_role(self, values):
        role = models.Role()
        role.update(values)
        try:
            role.save()
        except db_exc.DBDuplicateEntry as exc:
            raise exception.RoleAlreadyExists(name=values['name'])
        return role

    def get_role_by_name(self, role_name):
        query = model_query(models.Role).filter_by(name=role_name)
        try:
            return query.one()
        except NoResultFound:
            raise exception.RoleNotFound(role=role_name)

    def destroy_role(self, role_id):
        session = get_session()
        with session.begin():
            query = model_query(models.Role, session=session)
            query = query.filter_by(name=role_id)
    ...
    def update_role(self, role_id, values):
        try:
            return self._do_update_role(role_id, values)
        except db_exc.DBDuplicateEntry as e:
            if 'name' in e.columns:
                raise exception.RoleAlreadyExists(name=values['name'])
            else:
    ...
    def get_board_list(self, authorized_boards, filters=None, limit=None,
        ...
        query = self._add_boards_filters(query, filters, authorized_boards)
        return _paginate_query(models.Board, limit, marker,
                               sort_key, sort_dir, query)
    ...
    def get_plugin_list(self, authorized_plugins, filters=None, limit=None,
                        marker=None, sort_key=None, sort_dir=None):
        query = model_query(models.Plugin)
        query = self._add_plugins_filters(query, filters, authorized_plugins)
    ...
    def get_injection_plugin_list(self, board_uuid, authorized_board_plugins):
        query = model_query(models.InjectionPlugin).filter_by(
            board_uuid=board_uuid)
        query = query.filter(
            models.InjectionPlugin.board_uuid.in_(authorized_board_plugins))
        return query.all()
    ...
    def get_service_list(self, authorized_services, filters=None, limit=None,
                         marker=None, sort_key=None, sort_dir=None):
        query = model_query(models.Service)
        query = self._add_services_filters(query, filters, authorized_services)
    ...
    def get_port_list(
            self, authorized_ports, filters=None, limit=None, marker=None,
            sort_key=None, sort_dir=None):
        query = model_query(models.Port)
        query = self._add_ports_filters(query, filters, authorized_ports)
    ...
    def get_fleet_list(self, authorized_fleets, filters=None, limit=None,
                       marker=None, sort_key=None, sort_dir=None):
        query = model_query(models.Fleet)
        query = self._add_fleets_filters(query, filters, authorized_fleets)
    ...
    def get_webservice_list(self, authorized_webservices, filters=None,
                            limit=None, marker=None,
                            sort_key=None, sort_dir=None):
        query = model_query(models.Webservice)
        query = self._add_webservices_filters(
            query, filters, authorized_webservices)
    ...
    def get_enabled_webservice_list(self, authorized_board_webservices,
                                    filters=None, limit=None, marker=None,
                                    sort_key=None, sort_dir=None):
        query = model_query(models.EnabledWebservice)
        query = self._add_enabled_webservices_filters(
            query, filters, authorized_board_webservices)
