...
def user_list(request):
    """List users."""
    return iotronicclient(request).user.list()


def user_get(request, user_id, fields=None):
    """Get user info."""
    return iotronicclient(request).user.get(user_id, fields)


def user_create(request, params):
    """Create user."""
    iotronicclient(request).user.create(**params)


def user_update(request, user_id, patch):
    """Update user."""
    iotronicclient(request).user.update(user_id, patch)


def user_delete(request, user_id):
    """Delete user."""
    iotronicclient(request).user.delete(user_id)
...
def role_list(request):
    """List roles."""
    return iotronicclient(request).role.list()


def role_get(request, role_id, fields=None):
    """Get role info."""
    return iotronicclient(request).role.get(role_id, fields)


def role_get_operations(request):
    """Get operations list."""
    return iotronicclient(request).role.get_operations()


def role_create(request, params):
    """Create role."""
    iotronicclient(request).role.create(**params)


def role_update(request, role_id, patch):
    """Update role."""
    iotronicclient(request).role.update(role_id, patch)


def role_delete(request, role_id):
    """Delete role."""
    iotronicclient(request).role.delete(role_id)
...
def node_delegation_list(request, node_id, type):
    """List delegations."""
    return iotronicclient(request).delegation.list(node_id, type)


def node_delegation_get(request, dele_id):
    """Get delegation info."""
    return iotronicclient(request).delegation.get(dele_id)


def node_create_delegation(request, node_id, params):
    """Create node delegation."""
    iotronicclient(request).delegation.create(node_id, **params)


def node_update_delegation(request, dele_id, params):
    """Update node delegation."""
    iotronicclient(request).delegation.update(dele_id, **params)


def node_delete_delegation(request, dele_id):
    """Delete node delegation."""
    iotronicclient(request).delegation.delete(dele_id)
