...
class BoardDelegateView(forms.ModalFormView):
    template_name = 'iot/delegatenode.html'
    modal_header = _("Delegate a board")
    form_id = "delegate_board"
    form_class = project_forms.BoardDelegateForm
    submit_label = _("Delegate")
    submit_url = "horizon:iot:boards:delegateboard"
    success_url = reverse_lazy('horizon:iot:boards:index')
    page_title = _("Delegate board {{board.name}}")
    ...
    def get_users(self):
        try:
            return api.iotronic.user_list(self.request)
        except Exception as ex:
            redirect = reverse("horizon:iot:boards:index")
            exceptions.handle(self.request,
    ...
    def get_context_data(self, **kwargs):
        context = super(BoardDelegateView, self).get_context_data(**kwargs)
        args = (self.get_object().uuid,)
        context['submit_url'] = reverse(self.submit_url, args=args)
        return context
...
class DelegationUpdateView(forms.ModalFormView):
    ...
    modal_header = _("Update Board Delegation")
    form_id = "update_board_delegation_form"
    form_class = project_forms.UpdateBoardDelegationForm
    submit_label = _("Update Board Delegation")
    submit_url = "horizon:iot:boards:delegationupdate"
    # success_url = reverse_lazy('horizon:iot:boards:delegations')
    page_title = _("Update Board Delegation")
    base_node = None

    def get_success_url(self, **kwargs):
        return reverse('horizon:iot:boards:delegations',
                       args={self.base_node: 'board_id'})
    ...
    def get_delegation(self):
        try:
            ...
            return api.iotronic.node_delegation_get(self.request, dele)
        except Exception as ex:
            exceptions.handle(self.request,
...
class DelegationDeleteView(forms.ModalFormView):
    ...
    modal_header = _("Delete Board Delegation")
    form_id = "delete_board_delegation_form"
    form_class = project_forms.DeleteBoardDelegationForm
    submit_label = _("Delete Board Delegation")
    submit_url = "horizon:iot:boards:delegationdelete"
    # success_url = reverse_lazy('horizon:iot:boards:delegations')
    page_title = _("Delete Board Delegation")
    base_node = None
...
class DelegationsView(tables.DataTableView):
    ...
    def get_data(self):
        delegation = []
        try:
            delegation = api.iotronic.node_delegation_list(
                self.request, self.kwargs['board_id'], 'board')
        except Exception as ex:
            exceptions.handle(self.request,
...
class BoardDelegationsView(DelegationsView):
    redirect_url = 'horizon:iot:boards:index'
