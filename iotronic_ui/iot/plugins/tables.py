...
class DelegateLink(tables.LinkAction):
    name = "delegateplugin"
    verbose_name = _("Delegate Plugin")
    url = "horizon:iot:plugins:delegateplugin"
    classes = ("ajax-modal",)
    icon = "plus"
...
class PluginsTable(tables.DataTable):
    ...
    delegations = tables.WrappingColumn('delegations',
                                        link="horizon:iot:plugins:delegations",
                                        verbose_name=_(''))
    ...
    def get_row_actions(self, datum):
        ...
        for action in actions:
            ...
            elif datum.callable is True and action.name == "call":
                selected_row_actions.append(action)

            elif datum.callable is False and action.name != "call":
                selected_row_actions.append(action)
    ...
    class Meta(object):
        ...
        row_actions = (EditPluginLink, DelegateLink, InjectPluginLink,
                       StartPluginLink,
...
class EditDelegationLink(tables.LinkAction):
    name = "edit"
    verbose_name = _("Edit")
    url = "horizon:iot:plugins:delegationupdate"
    classes = ("ajax-modal",)
    icon = "pencil"
...
class DeleteDelegationsAction(tables.LinkAction):
    ...
    url = "horizon:iot:plugins:delegationdelete"
    classes = ("ajax-modal",)
    icon = "trash"
    action_type = "danger"
...
class DelegationsTable(tables.DataTable):
    ...
    node = tables.Column('node', verbose_name=_('Plugin'))
