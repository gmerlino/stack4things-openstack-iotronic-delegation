...
class PluginDelegateView(forms.ModalFormView):
    template_name = 'iot/delegatenode.html'
    modal_header = _("Delegate a plugin")
    form_id = "delegate_plugin"
    form_class = project_forms.PluginDelegateForm
    submit_label = _("Delegate")
    submit_url = "horizon:iot:plugins:delegateplugin"
    success_url = reverse_lazy('horizon:iot:plugins:index')
    page_title = _("Delegate plugin {{plugin.name}}")

    @memoized.memoized_method
    def get_object(self):
        try:
            return iotronic.plugin_get(self.request,
    ...
    def get_context_data(self, **kwargs):
        context = super(PluginDelegateView, self).get_context_data(**kwargs)
        args = (self.get_object().uuid,)
        context['submit_url'] = reverse(self.submit_url, args=args)
        return context
    ...
    def get_initial(self):
        ...
        return {'plugin_uuid': plugin.uuid,
                'roles': roles,
                'users': users}
...
class DelegationUpdateView(forms.ModalFormView):
    ...
    modal_header = _("Update Plugin Delegation")
    form_id = "update_plugin_delegation_form"
    form_class = project_forms.UpdatePluginDelegationForm
    submit_label = _("Update Plugin Delegation")
    submit_url = "horizon:iot:plugins:delegationupdate"
    # success_url = reverse_lazy('horizon:iot:plugins:delegations')
    page_title = _("Update Plugin Delegation")
    base_node = None

    def get_success_url(self, **kwargs):
        return reverse('horizon:iot:plugins:delegations',
                       args={self.base_node: 'plugin_id'})
...
class DelegationDeleteView(forms.ModalFormView):
    ...
    modal_header = _("Delete Plugin Delegation")
    form_id = "delete_plugin_delegation_form"
    form_class = project_forms.DeletePluginDelegationForm
    submit_label = _("Delete Plugin Delegation")
    submit_url = "horizon:iot:plugins:delegationdelete"
    # success_url = reverse_lazy('horizon:iot:plugins:delegations')
    page_title = _("Delete Plugin Delegation")
    base_node = None
    ...
    def get_data(self):
        ...
        try:
            delegation = iotronic.node_delegation_list(
                self.request, self.kwargs['plugin_id'], 'plugin')
        except Exception as ex:
            exceptions.handle(self.request,
...
class PluginDelegationsView(DelegationsView):
    redirect_url = 'horizon:iot:plugins:index'
