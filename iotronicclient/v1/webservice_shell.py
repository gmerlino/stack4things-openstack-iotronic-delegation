...
@cliutils.arg('webservice', metavar='<webservice>',
              help="Name or UUID of the webservice.")
def do_webservice_delegations(cc, args):
    """List webservice delegations."""

    fields = res_fields.DELEGATION_RESOURCE.fields
    delegations = cc.delegation.list(args.webservice, 'webservice')
    cliutils.print_list(delegations, fields,
                        sortby_index=None,
                        json_flag=args.json)
...
@cliutils.arg('webservice', metavar='<webservice>',
              help="Name or UUID of the webservice.")
@cliutils.arg('delegated', metavar='<delegated>',
              help="UUID of the user delegated.")
@cliutils.arg('role', metavar='<role>',
              help="Role to assign on this resource.")
def do_webservice_delegate(cc, args):
    """Register a delegation for a webservice resource."""

    field_list = ['delegated', 'role']
    fields = dict((k, v) for (k, v) in vars(args).items()
                  if k in field_list and not (v is None))
    fields['type'] = 'webservice'

    delegation = cc.delegation.create(args.webservice, **fields)
